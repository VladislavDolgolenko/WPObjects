...
class Contacts extends AbstractMetaBox
{
    public function __construct()
    {
        ...
        $this->setTemplatePath($template_dir . DIRECTORY_SEPARATOR . 'contacts.php');
    }
    ...
    {
        if (!isset($data['contact_counter']) || !is_array($data['contact_counter'])) {
            return array();
        }
        
        $social_counter = $data['contact_counter'];
        $_socials = array();
        foreach ($social_counter as $key => $count) {
            $social = array( 
                'icon_class' => sanitize_text_field($data['contact_icon_class'][$key]), 
                'link' => sanitize_text_field($data['contact_link'][$key]), 
                'name' => sanitize_text_field($data['contact_name'][$key])
            );
    ...
    public function getSocialsSelector($selected = null)
    {
        ...
        $Selector->setName('contact_icon_class')
                ->setOptions($options)
    ...
    {
        $elements = $this->getPostModel()->getMeta('_contacts');
